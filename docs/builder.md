...
![../_images/Builder.jpg](./assets/Builder.jpg)

## 4. 时序图

![../_images/seq_Builder.jpg](./assets/seq_Builder.jpg)

##  5. 代码分析

将 Builder 定义成抽象类，其中定义了四个抽象的涉及 Product 的构造国策划嗯方法，持有一个 Product 对象，并提供一个方法获取该对象。子类通过实现这几个抽象方法对继承来的 Product 对象设置属性。
...
ConcteBuilder 是 Builder 的具体实现，不同的 Builder 实现构造出来的对象可以不一样。
...
Director 作为指挥者，决定使用哪一种 Builder 实现和该实现构造对象的顺序。
...
```
Product{partA='ConcreteBuilder-style partA', partB='ConcreteBuilder-style partB', partC='ConcreteBuilder-style partC', partD='ConcreteBuilder-style partD'}
```
...
![../_images/KFCBuilder.jpg](./assets//KFCBuilder.jpg)
